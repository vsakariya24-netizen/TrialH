 ...
 let ms = null;
  ...
  return {
   ...
   const buildProductSchema = (
     ...
     // Pull specs for additionalProperty
     ...
     return JSON.stringify({
       ...
       "image": images,
       "sku": product.slug,
       "mpn": product.slug,          // Manufacturer Part Number fallback
       ...
       "size": selectedDia && selectedLen
         ? `${selectedDia} × ${selectedLen} ${selectedUnit}`
         : undefined,
       ...
       "offers": {
         ...
         "priceCurrency": "INR",
         "price": "0",                               // ← replace with real price if available
         "priceSpecification": {
   ...
   const [activeAppIndex, setActiveAppIndex] = useState(0);
     ...
     // ✅ Single source of truth – no duplicate /api fetch needed
     useEffect(() => {
       const fetchProduct = async () => {
         ...
         try {
           ...
           const fullProduct = {
             ...
             faqs:                     productData.faqs                     || [], // ← from Supabase jsonb column
           };
   ...
   if (!product)
     ...
     return (
       ...
       >
         {/* ====================================================================
             HEAD – Title, Meta, 3× Schema scripts
